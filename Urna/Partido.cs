...
namespace Urna
{
    public class Partido
    {
        ...
        public void separaCandidatos(List<Candidato> candidatos)
        {
            foreach(Candidato c in candidatos)
            {
                if (c.Partido == Nome)
                {
                    ...
                    {
                        vereadores.Add(c);
                        VotosTotaisVereador += c.QntVotos;
                    }
                    else if (c.Cargo == "Deputado Estadual")
                    {
                        depEstadual.Add(c);
                        VotosTotaisDepEstadual += c.QntVotos;

                    }
                    else if (c.Cargo == "Deputado Federal")
                    {
                        depFederal.Add(c);
                        VotosTotaisDepEstadual += c.QntVotos;
                    }
